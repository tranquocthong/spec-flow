...
5. **Re-implement**
   ...
   - Executor edits code. Narrative logging (`task-master update-task --id=<id> --append`) is opt-in behind `config.phase.taskNotes` (default `false`) — see `/sf:phase` step 3; `trace-link` is the durable record either way.
