...
const { evaluateRules } = require('./code-rules.cjs');
...
module.exports = {
  ...
  'verify-collect'(args) {
    ...
    // Tests the runner selected but could not execute (no request, no assertion,
    // no executing setup step). They are NOT evidence of anything, so they never
    // become a `verified` truth, and their presence holds the status at
    // `incomplete` — deliberately NOT `verified-adhoc`, which flow-tools'
    // VERIFIED_STATUS_RE accepts as shippable because it asserts a human DID
    // verify out of loop. Here nobody has yet. `incomplete` matches no gate, so
    // the ship guard stays shut until a human runs those cases, records the
    // evidence, and sets `verified-adhoc` themselves.
    const notVerified = (data.notVerified || []).map(n =>
      typeof n === 'string' ? { id: n, reason: 'not executed' } : { id: String(n.id || n), reason: String(n.reason || 'not executed') });

    const status = failed.length > 0 ? 'failed'
      : notVerified.length > 0 ? 'incomplete'
      : 'passed';
    ...
    const md = [
      ...
      `> Recorded by \`verify-collect\` from the checklist runner's result: ${passed.length} passed, ${failed.length} failed`
        + (notVerified.length ? `, ${notVerified.length} NOT verified (nothing to execute).` : '.'),
      ...(notVerified.length ? ['',
        `> \`status\` is \`incomplete\`, not \`passed\`: ${notVerified.length} test case(s) below were never executed,`,
        '> so this run is not evidence that they hold. Run them by hand, replace each line under',
        '> "Not verified" with what you observed, then set `status: verified-adhoc` yourself.',
        '> Do not mark them verified on the runner\'s word — it has none to give.'] : []),
      '',
      '## must_haves',
      '',
      '### truths',
      ...
      ...(notVerified.length ? ['', '## Not verified (never executed — needs a hand-run)', '',
        ...notVerified.map(n => `- ${n.id}: NOT VERIFIED — ${n.reason}`)] : []),
    ...
    return ok({ feature, verification: outPath, status, passed, failed, notVerified, truths, teardownWarnings });
  },
  ...
  //     forbiddenPatterns, rules, scanPath, secretScan }
  //
  ...
  //
  // Checks: tests · coverage · forbidden-patterns · code-rules · secret-scan
  //
  // code-rules (SD code-rules-gate FR-004..FR-006): `config.verify.rules`, evaluated
  // by lib/code-rules.cjs against the diff vs `config.branching.base` (default scope
  // "diff" per rule). Separate mechanism from forbiddenPatterns above, which keeps
  // its whole-scanPath behavior byte-for-byte (D2/NFR-001) — this is the opt-in
  // replacement for teams who want new rules to bite only on new/changed code.
  // --task/--files scoping (below) never narrows code-rules: it always evaluates
  // the full diff for this root, per FR-004.
  ...
  'verify-code'(args) {
    ...
    if (!verifyCfg) {
      const checks = [
        ...
        makeCheck('code-rules', 'skipped', 'No verify block in .spec-flow/config.json', null),
        makeCheck('secret-scan', 'skipped', 'No verify block in .spec-flow/config.json', null),
      ];
      const summary = { ok: 0, warn: 0, fail: 0, skipped: 5 };
    ...
    // Multi-repo with NO scope signal → we are about to scan every configured repo.
    // That is the documented fallback, but it is a bad silent outcome: on a 16-repo hub
    // it produces 64 check rows, mostly failures from repos this feature never touched,
    // with scope:null and no hint. It happens whenever `trace-repos --set` was skipped
    // AND trace-link stored bare paths (no --repo), which is the default if the flow's
    // multi-repo steps are not followed. Name the fix instead of shrugging.
    if (!repoFilter && roots.length > 1) {
      scopeWarnings.push(
        `UNSCOPED: no repo scope for this run, so all ${roots.length} configured repos will be checked — `
        + `an unrelated repo's red WIP can fail this feature's gate. Declare the target repos once with `
        + `\`trace-repos --feature ${vcFeature || '<feature>'} --set "repo-a,repo-b"\`, or pass \`--repos "repo-a,repo-b"\` for this run. `
        + `(Scoping also works from file-links when trace-link is given \`--repo <name>\`.)`
      );
    }
    ...
    // ---- d. code-rules ------------------------------------------------------
    // config.verify.rules (SD code-rules-gate FR-004..FR-006): diff-scoped rule
    // engine, separate from forbiddenPatterns above (which stays byte-identical —
    // D2/NFR-001). No rules configured -> skipped, never fail. A rule that fails
    // to validate or a diff that can't be resolved becomes a warning surfaced in
    // this check's detail — it NEVER fails the gate by itself (D4/NFR-003).
    if (!Array.isArray(codeRules) || codeRules.length === 0) {
      checks.push(makeCheck('code-rules', 'skipped',
        'config.verify.rules is empty — add diff-scoped rules there (see config.verify.rules).', null));
    } else {
      const base = (cfg.branching && cfg.branching.base) || 'main';
      const result = evaluateRules({ rootDir, scanPath, rules: codeRules, base });
      const warnSuffix = (result.warnings && result.warnings.length) ? `\nwarnings: ${result.warnings.join('; ')}` : '';
      if (result.skippedReason) {
        const reasonText = result.skippedReason === 'NO_VALID_RULES'
          ? 'no valid rules — every rule in config.verify.rules failed validation'
          : 'no applicable rules — diff scope unavailable and no scope: "all" rule configured';
        checks.push(makeCheck('code-rules', 'skipped', `${reasonText}.${warnSuffix}`, null));
      } else if (result.violations.length > 0) {
        const shown = result.violations.slice(0, 10).map((v) => `${v.file}:${v.line} [${v.rule}] ${v.message}`);
        let detail = `${result.violations.length} violation(s):\n${shown.join('\n')}`;
        if (result.violations.length > 10) detail += `\n... ${result.violations.length} total`;
        if (result.preexisting > 0) detail += `\npreexisting: ${result.preexisting}`;
        detail += warnSuffix;
        checks.push(makeCheck('code-rules', 'fail', detail,
          'Fix the flagged violations, or adjust the matching rule in config.verify.rules.'));
      } else {
        let detail = `No in-scope violations (checked ${codeRules.length} rule(s) in ${scanPath})`;
        if (result.preexisting > 0) detail += ` — preexisting: ${result.preexisting}`;
        detail += warnSuffix;
        checks.push(makeCheck('code-rules', 'ok', detail, null));
      }
    }
