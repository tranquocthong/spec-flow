...
module.exports = {
  ...
  'review-scope'(args) {
    ...
    //   projectAuthor — the team's own conventions, so house style is not a finding
  ...
  //   "blocking" — >=1 critical/high      → /sf:phase HALTS and asks the user
  // -----------------------------------------------------------------------
  'review-collect'(args) {
    ...
    if (blocking.length) {
      ...
      md.push('');
      md.push(`${blocking.length} blocking finding(s) (critical/high). \`/sf:phase\` halts the ship step and asks the user:`);
