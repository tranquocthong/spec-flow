...
module.exports = {
  ...
  'drift-check'(args) {
    ...
    if (!sourceText.trim()) {
      const scanRoot = path.resolve(process.cwd(), scanPath);
      sourceText = fs.existsSync(scanRoot) ? scanSource(scanRoot) : '';
      scopeNote = sourceText.trim() ? `scanPath "${scanPath}" (no file-links yet)` : '';
    }
    ...
    if (!sourceText.trim() && !implLog.trim()) {
      return ok({
    ...
    for (const re of tokenRes) { let m; while ((m = re.exec(haystack)) !== null) implCodes.add(m[0]); }

    const sdLcSet = new Set(sdCodes.map(lc));
    const where = sourceText.trim()
      ? `scanned ${scopeNote}${implLog.trim() ? ' + task logs' : ''}`
      : 'scanned task logs only (no source found)';
    const drift = [];
    // (1) SD-specified error code that exists nowhere we looked.
    for (const c of sdCodes) {
      if (!hayLc.includes(lc(c))) {
        drift.push({ type: 'spec-not-evidenced', code: c, hint: `SD §12.2 defines ${c} but it does not appear in the code (${where}) — implement it, or remove it from §12.2. Multi-repo features may hold it outside scanPath.` });
      }
    }
    ...
    return ok({
      ...
      evidencedErrorCodes: sdCodes.filter((c) => hayLc.includes(lc(c))),
