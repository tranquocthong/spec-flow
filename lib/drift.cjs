...
module.exports = {
  ...
  'drift-check'(args) {
    ...
    // Read BOTH fields. `update-task --append` (the /sf:phase logging op) writes
    // `notes`; `task-baseline --apply` writes `details`. Reading only `details` made
    // this source structurally blind to every note the implement loop ever wrote.
    const logParts = [];
    const take = (o) => { for (const k of ['details', 'notes']) if (o && o[k]) logParts.push(String(o[k])); };
    for (const t of tasks) {
      take(t);
      for (const st of (t && t.subtasks) || []) take(st);
    }
