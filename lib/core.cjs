...
const PATHS = {
  ...
  backlog: path.join(STATE_DIR, 'backlog'),
...
/**
 * Detect a plugin installed from a LOCAL DIRECTORY marketplace whose running copy
 * has drifted from that source directory.
 *
 * Claude Code does not run a directory-sourced plugin in place: it copies the
 * directory into ~/.claude/plugins/cache/<marketplace>/<plugin>/<version>/ and runs
 * THAT. The cache path is keyed by version, so while the version string holds still
 * — exactly what a careful maintainer does between releases — edits to the source
 * repo never reach the running plugin. The failure is silent and total: a new engine
 * command answers UNKNOWN_COMMAND, and an edited command doc is simply the old one.
 *
 * This only ever fires for a developer working ON the plugin. A normal install from
 * github has no directory source, so the check reports "not applicable" and stops.
 *
 * Pure: takes the already-parsed JSON so it is testable without touching $HOME.
 *
 * @param {string} pluginRoot - the RUNNING plugin dir (PLUGIN_ROOT)
 * @param {object|null} installedRaw   - parsed installed_plugins.json
 * @param {object|null} marketplacesRaw- parsed known_marketplaces.json
 * @param {(a: string, b: string) => {differing: string[], compared: number,
 *          missingSource?: boolean}} compare
 *        - injected tree comparison. ALL disk access lives here, including whether
 *          the source directory still exists, so the decision below is genuinely
 *          pure and testable with no temp dirs.
 * @returns {{status: string, detail: string, fix: string|null, source?: string}}
 */
function pluginSourceDrift(pluginRoot, installedRaw, marketplacesRaw, compare) {
  const na = (detail) => ({ status: 'ok', detail, fix: null });
  if (!installedRaw || !marketplacesRaw) return na('no local plugin install metadata to check against');

  const plugins = installedRaw.plugins && typeof installedRaw.plugins === 'object' ? installedRaw.plugins : {};
  const want = path.resolve(pluginRoot);
  let marketplaceName = null;
  for (const [key, entries] of Object.entries(plugins)) {
    for (const e of (Array.isArray(entries) ? entries : [])) {
      if (e && e.installPath && path.resolve(e.installPath) === want) {
        // key is "<plugin>@<marketplace>"
        const at = key.lastIndexOf('@');
        marketplaceName = at > 0 ? key.slice(at + 1) : null;
      }
    }
  }
  // Running from somewhere that is not an installed copy (a checkout, a test tmpdir)
  // means there is no cache to drift — the source IS what runs.
  if (!marketplaceName) return na('running from the source checkout, not an installed copy');

  const mp = marketplacesRaw[marketplaceName];
  const src = mp && mp.source && mp.source.source === 'directory' ? (mp.source.path || mp.installLocation) : null;
  if (!src) return na(`installed from marketplace "${marketplaceName}" (not a local directory source)`);
  if (path.resolve(src) === want) return na('the plugin runs directly from its source directory');

  const { differing, compared, missingSource } = compare(src, want);
  if (missingSource) {
    return { status: 'warn', detail: `source directory for "${marketplaceName}" no longer exists: ${src}`, fix: 're-add the marketplace, or ignore if the source repo moved', source: src };
  }
  if (!differing.length) {
    return { status: 'ok', detail: `running copy matches its source directory (${compared} file(s) compared)`, fix: null, source: src };
  }
  const shown = differing.slice(0, 5).join(', ');
  const more = differing.length > 5 ? `, +${differing.length - 5} more` : '';
  return {
    status: 'warn',
    detail: `the RUNNING plugin is stale: ${differing.length}/${compared} file(s) differ from the source directory (${shown}${more}). Edits to ${src} are NOT live — Claude Code runs the cached copy at ${want}, and the cache path is keyed by version, so it will not refresh while the version is unchanged.`,
    fix: `rsync -a --delete --exclude='.git/' "${src}/" "${want}/"   (then re-run doctor to confirm)`,
    source: src,
  };
}
